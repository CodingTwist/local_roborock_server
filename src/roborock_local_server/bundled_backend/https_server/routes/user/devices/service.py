...
def _product_from_device_record(raw_item: dict[str, Any]) -> dict[str, Any]:
    product_id = str(_get_value(raw_item, "product_id", "productId", default="")).strip()
    if not product_id:
        return {}
    product: dict[str, Any] = {
        "id": product_id,
        "name": str(_get_value(raw_item, "product_name", "productName", default="")),
        "model": str(_get_value(raw_item, "model", default="")),
        "category": str(_get_value(raw_item, "category", default="")),
        "code": str(_get_value(raw_item, "code", default="")),
        "iconUrl": str(_get_value(raw_item, "iconUrl", "icon_url", default="")),
    }
...
def _normalize_devices(
    ...
    *,
    ...
    for index, raw in enumerate(devices_raw):
        ...
        duid = str(_get_value(raw_item, "duid", "did", "device_id", "deviceId", default=fallback_duid)).strip()
        ...
        product_id = str(_get_value(raw_item, "product_id", "productId", default=f"product_{index + 1}")).strip()
        ...
        device = {
            ...
            "timeZoneId": timezone,
            "online": _as_bool(_get_value(raw_item, "online", default=True), True),
            "sn": str(
                _get_value(
                    raw_item,
                    "sn",
...
def resolve_home_id(*sources: dict[str, Any] | None, default: int = 0) -> int:
    for source in sources:
        ...
        for key in ("id", "rr_home_id", "rrHomeId", "home_id"):
            resolved = _as_int(source.get(key), 0)
            if resolved > 0:
                return resolved
    return default
...
def _runtime_device_identity_map(ctx: ServerContext) -> tuple[dict[str, dict[str, Any]], bool]:
    try:
        inventory_vacuums = _extract_inventory_vacuums(ctx, _load_inventory(ctx))
    ...
    for item in merged_vacuums:
        if not isinstance(item, dict):
            continue
        connected = _as_bool(item.get("connected"), False)
...
def _apply_runtime_online_status(ctx: ServerContext, home_data: dict[str, Any]) -> dict[str, Any]:
    ...
    for collection_key in ("devices", "receivedDevices", "received_devices"):
        ...
        for device in devices:
            ...
            updated_device["online"] = _as_bool(runtime_device.get("connected"), False)
...
def _home_data(ctx: ServerContext) -> dict[str, Any]:
    ...
    home["name"] = str(_get_value(home, "name", "home_name", default="Local Home"))
...
def device_detail_payload(ctx: ServerContext, device_id: str) -> dict[str, Any]:
    ...
    runtime_online = _as_bool(runtime_device.get("connected"), False)
    inventory_online = _as_bool(
    ...
    payload: dict[str, Any] = {
        ...
        "iconUrl": str(
            _get_value(
                ...
                "iconUrl",
                "icon_url",
                default=_get_value(normalized_device, "iconUrl", "icon_url", default=""),
            )
        ),
        "lon": _get_value(raw_device, "lon", default=None),
        "lat": _get_value(raw_device, "lat", default=None),
        "online": resolved_online,
        "share": _as_bool(_get_value(raw_device, "share", default=_get_value(normalized_device, "share", default=False)), False),
        "shareTime": _get_value(
            raw_device,
            "shareTime",
            "share_time",
            default=_get_value(normalized_device, "shareTime", "share_time", default=None),
        ),
        "tuyaMigrated": _as_bool(
            _get_value(raw_device, "tuyaMigrated", default=_get_value(normalized_device, "tuyaMigrated", default=False)),
            False,
        ),
        "extra": _get_value(raw_device, "extra", default=_get_value(normalized_device, "extra", default="{}")) or "{}",
        "sn": str(_get_value(raw_device, "sn", default=_get_value(normalized_device, "sn", default=""))),
        "deviceStatus": _get_value(
            raw_device,
            "deviceStatus",
            default=_get_value(normalized_device, "deviceStatus", default={}),
        )
        or {},
        "silentOtaSwitch": _as_bool(
            _get_value(
                raw_device,
                "silentOtaSwitch",
                default=_get_value(normalized_device, "silentOtaSwitch", default=False),
            ),
            False,
        ),
        "f": _as_bool(_get_value(raw_device, "f", default=_get_value(normalized_device, "f", default=False)), False),
...
def device_jobs_payload(ctx: ServerContext, device_id: str) -> list[dict[str, Any]]:
    schedules_map = _load_inventory(ctx).get("schedules")
